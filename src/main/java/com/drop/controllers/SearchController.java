...
public class SearchController extends BaseController {
	...
	public ModelAndView getMatchingDeals(
		...
		try {
			...
			if (searchType.equals("Drops")) {
				dealPostListForSearch = solrSearchService.searchDrops(
						searchStr, 0, SORT_TYPE.PRICE,0);
				map.addAttribute("dealPostListForSearch", dealPostListForSearch);
				searchDealForm.setItemsOnpage(dealPostListForSearch.size());
			} else {
				dealWantedListForSearch = solrSearchService.searchWanted(
						searchStr, 0, SORT_TYPE.PRICE, 0);
	...
	public ModelAndView searchByCategory(
			@RequestParam("categoryId") long categoryId, ModelMap map) {

		ModelAndView modelAndView = new ModelAndView("searchResults");

		try {

			SearchDealForm sessionDealForm = (SearchDealForm) session
					.getAttribute("sessionSearchDealForm");
			...
			sessionDealForm.setSelectedCategory(categoryId);

			if (searchType.equals("Drops")) {
				dealPostListForSearch = solrSearchService.searchDrops(
						searchStr, currentPageNo, sortType, categoryId);
				map.addAttribute("dealPostListForSearch", dealPostListForSearch);
	...
	public ModelAndView sortSearch(@RequestParam("sortBy") String sortBy,
			ModelMap map) {
		...
		try {
			...
			String searchType = sessionDealForm.getSearchType();
			String searchStr = sessionDealForm.getSearchString();
			long categoryId = sessionDealForm.getSelectedCategory();
			int currentPageNo = sessionDealForm.getCurrentPage();
	...
	public ModelAndView searchByCategory(@RequestParam("pageNo") int pageNo,
			ModelMap map) {
		...
		try {
			...
			if (searchType.equals("Drops")) {
				dealPostListForSearch = solrSearchService.searchDrops(
						searchStr, pageNo, sortType, categoryId);
				map.addAttribute("dealPostListForSearch", dealPostListForSearch);
				sessionDealForm.setItemsOnpage(dealPostListForSearch.size());
			} else {
				dealWantedListForSearch = solrSearchService.searchWanted(
